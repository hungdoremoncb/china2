<?php /*a:1:{s:69:"/www/wwwroot/103.77.173.23/application/index/view/my/signin-blue.html";i:1657585222;}*/ ?>
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="maximum-scale=1.0,minimum-scale=1.0,user-scalable=0,width=device-width,initial-scale=1.0" /><meta name="format-detection" content="telephone=no"><title><?php echo lang("签到"); ?></title><link rel="stylesheet" href="/sigin/css/public.css" /><link rel="stylesheet" href="/sigin/css/signin-blue.css" /><link rel="stylesheet" href="/static_new/css/public.css"><script charset="utf-8" src="/static_new/js/jquery.min.js"></script><script charset="utf-8" src="/static_new/js/dialog.min.js"></script><style>        .footer ul li p[data-v-59e03106]{font-size: .6rem!important;}
        body {padding-top: unset!important;}
    </style></head><body><div class="top flex flex-align-end flex-pack-center flex-warp"><div class="out-1 flex flex-align-center flex-pack-center" id="signIn"><div class="out-2 flex flex-align-center flex-pack-center"><div class="signBtn"><strong id="sign-txt"><?php echo lang("签到"); ?></strong><span><?php echo lang("连续"); ?><em id="sign-count">0</em><?php echo lang("天"); ?></span></div></div></div></div><div class="tips"><?php echo lang("今天已签到，获得1个红包"); ?></div><div class="Calendar"><div id="toyear" class="flex flex-pack-center"><div id="idCalendarPre">&lt;</div><div class="year-month"><span id="idCalendarYear">2018</span><?php echo lang("年"); ?><span id="idCalendarMonth">6</span><?php echo lang("月"); ?></div><div id="idCalendarNext">&gt;</div></div><table border="1px" cellpadding="0" cellspacing="0"><thead><tr class="tou"><td><?php echo lang("日"); ?></td><td><?php echo lang("一"); ?></td><td><?php echo lang("二"); ?></td><td><?php echo lang("三"); ?></td><td><?php echo lang("四"); ?></td><td><?php echo lang("五"); ?></td><td><?php echo lang("六"); ?></td></tr></thead><tbody id="idCalendar"></tbody></table></div><div style="margin:0 auto;text-align:center;background-color:#fff;padding-bottom:5rem"><table border="1px" cellpadding="1" cellspacing="1" style="margin: auto;width:90%"><thead><tr class="tou"><td><?php echo lang("签到时间"); ?></td><td><?php echo lang("获得红包"); ?>(<?php echo lang("￥"); ?>)</td></tr></thead><tbody ><?php if(is_array($signlog) || $signlog instanceof \think\Collection || $signlog instanceof \think\Paginator): $i = 0; $__LIST__ = $signlog;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tou"><td><?php echo date('Y-m-d H:i:s', $vo['signtime']);; ?></td><td><?php echo htmlentities($vo['money']); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></div><script type="text/javascript" src="/sigin/js/rili.js"></script><script language="JavaScript">			var isSign = false;
			var myday = new Array(); //已签到的数组
            <?php if($signlist): if(is_array($signlist) || $signlist instanceof \think\Collection || $signlist instanceof \think\Paginator): $i = 0; $__LIST__ = $signlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>                    myday[<?php echo htmlentities($i-1); ?>] = <?php echo htmlentities($v); ?>;
                <?php endforeach; endif; else: echo "" ;endif; ?><?php endif; ?>			var cale = new Calendar("idCalendar", {
				qdDay: myday,
				onToday: function(o) {
					o.className = "onToday";
				},
				onSignIn: function (){
					$$("sign-txt").innerHTML = '<?php echo lang("已签到"); ?>';
				},
				onFinish: function() {
					$$("sign-count").innerHTML = myday.length //已签到次数
					$$("idCalendarYear").innerHTML = this.Year;
					$$("idCalendarMonth").innerHTML = this.Month; //表头年份

				}
			});
			$$("idCalendarPre").onclick = function() {
				cale.PreMonth();
			}
			$$("idCalendarNext").onclick = function() {
				cale.NextMonth();
			}
			//添加今天签到
			$$("signIn").onclick = function() {
		var now=new Date();
		$.ajax({
        type:'post',
        url:'/index/my/do_signin',
        data:{time:now},
        dataType:'json',
        success:function(res){
             $(document).dialog({infoText: res.info,autoClose:2000});
             
         console.log(res);
         var linkTime = setTimeout(function() {
                                    location.reload()
                                }, 1800);
        }
      });
			  

			}
			
					var res = cale.SignIn();
					console.log(res);
		</script><style>		    .footer[data-v-59e03106] {
		        height: 3.6rem !important;
    position: fixed;
    left: 0;
    bottom: 0;
    width: 100%;
    color: #757575;
    background: url(data:image/png;base64,);
    background-repeat: no-repeat;
    background-size: 100% auto;
    -moz-background-size: 100% auto;
    background-position: bottom;
    z-index: 999;
}
.footer ul[data-v-59e03106] {
    overflow: hidden;
    padding-bottom: 2rem;
}

.footer ul li[data-v-59e03106] {
    float: left;
    width: 2%;
    padding-top: 1.2rem !important;
    height: 2.466667rem !important
}

.footer ul li img[data-v-59e03106] {
    width: auto;
    height: 1.2rem !important;
    display: block;
    margin: 0 auto;
    margin-bottom: .066667rem
}

.footer ul li.on img[data-v-59e03106]:first-child, .footer ul li img[data-v-59e03106]:nth-child(2) {
    display: none
}

.footer ul li.on img[data-v-59e03106]:nth-child(2) {
    display: block
}



.footer ul li.on p[data-v-59e03106] {
    color: blue;
}

.footer ul .t[data-v-59e03106] {
    padding-top: .533333rem !important
}

.footer ul .t img[data-v-59e03106] {
    height: 3.6rem !important;
    margin-top: -.666667rem !important
}
		    
		</style><link href="/web/css/blue.css" rel="stylesheet" /><div data-v-59e03106="" data-v-43c1797b="" class="footer"><ul data-v-59e03106=""><li data-v-59e03106="" class="" onclick="window.location.href='<?php echo url('index/home'); ?>'"><img data-v-59e03106="" src="data:image/png;base64," alt="" /><img data-v-59e03106="" src="/public/logo1.png" alt="" /><p data-v-59e03106=""><?php echo lang('首页'); ?></p></li><li data-v-59e03106="" class="" onclick="window.location.href='<?php echo url('order/index'); ?>'"><img data-v-59e03106="" src="data:image/png;base64," alt="" /><img data-v-59e03106="" src="/public/logo2.png" alt="" /><p data-v-59e03106=""><?php echo lang('记录'); ?></p></li><li data-v-59e03106="" class="t" onclick="window.location.href='<?php echo url('rot_order/index'); ?>'"><img data-v-59e03106="" src="<?php echo lang('/public/footico.png'); ?>" alt="" /><img data-v-59e03106="" src="<?php echo lang('/public/footico.png'); ?>" alt="" /></li><li data-v-59e03106="" class="" onclick="window.location.href='<?php echo url('support/index'); ?>'"><img data-v-59e03106="" src="data:image/png;base64," alt="" /><img data-v-59e03106="" src="/public/logo4.png" alt="" /><p data-v-59e03106=""><?php echo lang('在线'); ?></p></li><li data-v-59e03106="" class="" onclick="window.location.href='<?php echo url('my/index'); ?>'"><img data-v-59e03106="" src="data:image/png;base64," alt="" /><img data-v-59e03106="" src="/public/logo5.png" alt="" /><p data-v-59e03106=""><?php echo lang('我的'); ?></p></li></ul></div></body><script>      $(function() {
        $('.footer li').eq(4).addClass("on");

    })
</script></html>